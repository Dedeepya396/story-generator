...
class CharacterSelectionPage extends StatefulWidget {
  // 👇 Flag to know if opened from WriteStoryPage or directly
  final bool isFromWritePage;

  const CharacterSelectionPage({
    super.key,
    this.isFromWritePage = false,
  });
...
class _CharacterSelectionPageState extends State<CharacterSelectionPage> {
  final List<Map<String, String>> characters = [
    {'name': 'Lion', 'image': 'assets/images/characters/lion.png'},
    {'name': 'Rabbit', 'image': 'assets/images/characters/rabbit.png'},
    {'name': 'Elephant', 'image': 'assets/images/characters/elephant.png'},
    {'name': 'Turtle', 'image': 'assets/images/characters/tortoise.png'},
    {'name': 'Fox', 'image': 'assets/images/characters/fox.png'},
  ];
          ...
          children: [
            ...
            if (selectedIndexes.isNotEmpty)
              Padding(
                ...
                child: Text(
                  '${selectedIndexes.length} character${selectedIndexes.length > 1 ? 's' : ''} selected',
                  style: const TextStyle(
                    fontSize: 14,
                    color: Colors.blue,
                    fontWeight: FontWeight.w600,
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  final selectedCharacters =
                      selectedIndexes.map((i) => characters[i]).toList();

                  if (widget.isFromWritePage) {
                    // 👇 Pop back to WriteStoryPage with selected characters
                    Navigator.pop(context, selectedCharacters);
                  } else {
                    // 👇 Direct navigation (entry point)
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => WriteStoryPage(
